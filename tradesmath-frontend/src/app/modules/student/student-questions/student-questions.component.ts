...
export class StudentQuestionsComponent implements OnInit {
    ...
    onSubmit(formValue): void {
        if (
            ...
            const questiondata: any = window._.cloneDeep(
                Object.values(this.questionAnswerForm.value)
            );
            const answers = ['equal', 'greater', 'lesser'];
            questiondata.forEach((element) => {
                if (element.answer) {
                    element.answer = element.answer.toLowerCase().trim();
                }
                if (!answers.includes(element.answer)) {
                    element.answer = element.answer.replace(/[^-0-9.\/]/g, '');
                }
            });
            this.data = [];
            const dataPayload = [];
            for (let i = 0; i <= this.questions.length - 1; i++) {
                ...
                dataPayload.push({
                    ...
                    questionTemplate: this.questions[i].questionText,
                    expectedAnswer: this.questions[i].answer,
                    userAnswer: questiondata[i].answer,
                    equation: this.questions[i].equation,
                    equationSimplified: this.questions[i].equationSimplified,
                    questionId: this.questions[i]._id,
                    isCorrect:
                        questiondata[i].answer &&
                        this.questions[i].answer == questiondata[i].answer,
                });
                this.data.push(tempdata);
            }
            this._analyticsService.saveAnalytics({
                questions: dataPayload,
                topicId: this.questionKey,
                score: dataPayload.filter(
                    (x: any) => x.expectedAnswer == x.userAnswer
                ).length,
            });
    ...
    // goNext(): void {
    //   const currentIndex = this.horizontalStepper.selectedIndex;
    //   // const validAnswer = this.questions[currentIndex].answer;
    //   const currentAnswer = this.questionAnswerForm.value[
