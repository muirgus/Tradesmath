    ...
    implements OnChanges, OnInit, AfterViewInit, OnDestroy 
{
    ...
    userWiseLogin(){
       const data =  atob(localStorage.getItem('info'));
    ...
    canAccessUrl(roleInfo: string): boolean {
        if (roleInfo) {
            const differentRoles = roleInfo.split(',');
            if (
                this.user.isSuperAdmin &&
                differentRoles.includes(appRolesInfo.superAdmin)
            ) {
                return true;
            } else if (
                this.user.isAdmin &&
                differentRoles.includes(appRolesInfo.admin)
            ) {
                return true;
            } else if (
                !this.user.isAdmin &&
                !this.user.isSuperAdmin &&
                differentRoles.includes(appRolesInfo.user)
            ) {
                return true;
            }
        }
