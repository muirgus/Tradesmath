...
export class AuthUtils {
    ...
    static getUsersRoute(): Array<string> {
        return [
            '/student',
            '/student/topics',
            '/student/questions',
    ...
    static ignoreUrls(): Array<string> {
        return [
            '/',
            '/sign-in',
            '/sign-out',
    ...
    static getAdminRoute(): Array<string> {
        return [
            '/app/dashboard',
            '/app/topics',
            '/app/view-topics',
    ...
    static getSuperAdminRoute(): Array<string> {
        return [
            '/app/generate-invitation',
            '/app/users',
            '/app/users-details',
            '/sign-out',
            '/sign-in',
        ...
        try {
            if (localStorage.getItem('info')) {
                const userInfo = JSON.parse(
                    atob(localStorage.getItem('info'))
                ) as User;
                return userInfo;
            }
            return null;
    ...
    private static _decodeToken(token: string): any {
        ...
        if (parts.length !== 3) {
            throw new Error(
                'The inspected token doesnt appear to be a JWT. Check to make sure it has three parts and see https://jwt.io for more.'
            );
        }
